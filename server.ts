...
if (!OPENROUTER_API_KEY) {
  console.warn('[WARN] OPENROUTER_API_KEY is not set — AI will use heuristic fallback only');
}
...
// ─── PDF text extraction (no library — no ESM/CJS issues) ────────────────────
...
function heuristicParseResume(rawText: string, fileName: string) {
  const text = rawText || '';
  ...
  const locationMatch = text.match(/([A-Z][a-zA-Z\s]+,\s*(?:[A-Z]{2}|[A-Z][a-z]+)(?:,\s*[A-Z][a-z]+)?)/);
  ...
  const expSection = text.match(/(?:work\s*)?experience[:\s]+([\s\S]{0,800}?)(?:\n{2,}|education|skills|projects|$)/i)?.[1] || '';
  const companies  = expSection
    .split('\n').map((l) => l.trim())
    .filter((l) => l.length > 2 && l.length < 60 && /[A-Z]/.test(l) && !/^\d/.test(l))
    .slice(0, 5);

  const eduSection    = text.match(/education[:\s]+([\s\S]{0,400}?)(?:\n{2,}|experience|skills|projects|$)/i)?.[1] || '';
...
// ─── Heuristic screening scorer (no AI) ──────────────────────────────────────
...
// ─── AI evaluation — OpenRouter with heuristic fallback ──────────────────────

async function runAiEvaluation(job: Job, candidate: Candidate): Promise<Partial<Screening>> {
  if (!OPENROUTER_API_KEY) {
    console.warn('[SCREENING] No OPENROUTER_API_KEY — using heuristic');
...
Use exactly this structure:
{
  ...
  "experienceAlignment": "<sentence>",
  "educationAlignment": "<sentence>",
  "redFlags": ["flag1"],
  "biasIndicators": {
    "ageRisk": false,
    "genderRisk": false,
  ...
  "proposedQuestions": ["question1", "question2"],
  "estimatedOnboarding": "<Easy|Moderate|Challenging>",
...
Resume excerpt: ${(candidate.resumeText || '').slice(0, 800)}`;
    ...
    console.warn('[SCREENING FALLBACK] OpenRouter failed:', err?.message?.slice(0, 100));
    return heuristicEval(candidate, job);
  }
}
...
async function autoScheduleCandidateInterview(candidateId: string, jobId: string) {
  ...
  if (pRow) {
    const history = [...(pRow.stage_history ?? []), {
      stage: 'interview', changedAt: new Date().toISOString(),
      changedBy: 'AI Recruiter Bot', notes: 'Auto-scheduled interview.'
    }];
    await supabase.from('pipeline').update({ current_stage: 'interview', stage_history: history })
...
function startServer() {
  ...
  app.get('/api/status', (_req, res) => {
    res.json({
      ...
      mode: OPENROUTER_API_KEY ? `OpenRouter (${OPENROUTER_MODEL})` : 'Heuristic Fallback',
    });
  });

  // ── PARSE RESUME ─────────────────────────────────────────────────────────────
  // Always returns HTTP 200 — never crashes the upload flow.
  // Flow: extract text from PDF bytes → OpenRouter → heuristic → filename fallback
  app.post('/api/parse-resume', async (req, res) => {
    const { fileBase64, fileName } = req.body;
    const safeFileName = fileName || 'resume.pdf';

    // No file data at all — return safe empty candidate
    if (!fileBase64) {
      console.warn('[PARSE-RESUME] No fileBase64 received');
      return res.status(200).json({
        name: safeFileName.replace(/\.[^/.]+$/, '').replace(/[_-]/g, ' ').trim(),
        email: '', phone: '', location: '',
        skills: [], experienceYears: 0, companies: [],
        education: { degree: '', field: '', school: '', graduationYear: 0 },
        resumeText: '', parsedByFallback: true,
      });
    }

    // Step 1 — extract readable text from PDF bytes (no library needed)
    const rawText = extractTextFromBase64Pdf(fileBase64);

    if (!rawText || rawText.length < 50) {
      console.warn('[PARSE-RESUME] Could not extract readable text — using heuristic');
      return res.status(200).json(heuristicParseResume('', safeFileName));
    }
...
Use exactly this structure (empty string or 0 for missing fields, empty array for missing lists):
{
  "name": "Full Name",
  ...
  "experienceYears": 3,
  "companies": [{"company": "Name", "role": "Title", "duration": "2 years"}],
  "education": {
    "degree": "B.Tech",
  ...
  "resumeText": "first 300 chars of resume text"
}
...
${rawText.slice(0, 4000)}`;
        ...
        return res.status(200).json({
          name:            parsed.name            || heuristicParseResume(rawText, safeFileName).name,
          email:           parsed.email           || '',
          phone:           parsed.phone           || '',
          ...
          resumeText:      parsed.resumeText || rawText.slice(0, 500),
          parsedByFallback: false,
        });

      } catch (err: any) {
        console.warn('[PARSE-RESUME FALLBACK] OpenRouter failed:', err?.message?.slice(0, 100));
      }
    } else {
      console.warn('[PARSE-RESUME] No OPENROUTER_API_KEY — using heuristics');
    }
  ...
  app.post('/api/jobs', async (req, res) => {
    const b = req.body;
    const row = {
      id: `job-${Date.now()}`, title: b.title || 'Untitled Job', department: b.department || 'General',
      location: b.location || 'Remote', description: b.description || '',
      required_skills: b.requiredSkills ?? [], nice_to_have_skills: b.niceToHaveSkills ?? [],
      experience_years: Number(b.experienceYears) || 0, salary_min: Number(b.salaryMin) || 0,
      salary_max: Number(b.salaryMax) || 0, currency: 'USD', status: 'active',
      created_at: new Date().toISOString(), strictness: Number(b.strictness) || 50,
  ...
  app.post('/api/candidates', async (req, res) => {
    ...
    const row = {
      id, name: b.name || 'Anonymous', email: b.email || '', phone: b.phone || '',
      location: b.location || 'Remote', skills: b.skills ?? [],
      experience_years: Number(b.experienceYears) || 0, companies: b.companies ?? [],
  ...
  app.post('/api/candidates/bulk', async (req, res) => {
    ...
    const candRows = list.map((item: any, i: number) => ({
      ...
      experience_years: Number(item.experienceYears) || 3, companies: item.companies ?? [],
  ...
  app.post('/api/candidates/:id/blacklist', async (req, res) => {
    ...
    const { data, error } = await supabase.from('candidates')
      .update({ blacklisted: isBlacklisted, blacklist_reason: isBlacklisted ? (reason || 'Flagged for recruitment violation') : '' })
    ...
    if (isBlacklisted) {
      ...
      for (const p of pipes ?? []) {
        ...
        await supabase.from('pipeline').update({ current_stage: 'rejected', decision: 'rejected', stage_history: history })
          .eq('candidate_id', id).eq('job_id', p.job_id);
      }
    }
  ...
  app.post('/api/screenings/evaluate', async (req, res) => {
    ...
    try {
      ...
      const screenRow = {
        id: `scr-${Date.now()}`, candidate_id: candidateId, job_id: jobId,
      ...
      if (pRow) {
        ...
        await supabase.from('pipeline').update({ current_stage: stage, stage_history: history })
          .eq('candidate_id', candidateId).eq('job_id', jobId);
  ...
  app.post('/api/interviews', async (req, res) => {
    ...
    const row = {
      ...
      date: b.date || new Date().toISOString().split('T')[0], time: b.time || '12:00',
      type: b.type || 'video', interviewer: b.interviewer || 'Recruiter',
      status: 'scheduled', meeting_link: b.meetingLink || 'https://meet.google.com/meet-link'
    };
    ...
    if (pRow && pRow.current_stage !== 'interview') {
      ...
      await supabase.from('pipeline').update({ current_stage: 'interview', stage_history: history })
        .eq('candidate_id', b.candidateId).eq('job_id', b.jobId);
    }
  ...
  app.post('/api/interviews/:id/feedback', async (req, res) => {
    ...
    const { data, error } = await supabase.from('interviews')
      .update({ status: 'completed', rating: Number(rating) || 5, feedback })
    ...
    if (pRow) {
      ...
      await supabase.from('pipeline').update({ interview_feedback: feedback, stage_history: history })
        .eq('candidate_id', int.candidateId).eq('job_id', int.jobId);
    }
    res.json(int);
  });
  ...
  app.post('/api/gmail/send-custom', async (req, res) => {
    ...
    const row = {
      ...
      type: 'email', subject: subject || 'Recruitment Update', body: body || '',
      status: connected ? 'sent' : 'pending', sent_at: new Date().toISOString()
    };
  ...
  app.get('/api/analytics', async (req, res) => {
    ...
    res.json({
      ...
      rates: {
        ...
        interviewRate: count(['shortlisted','interview','offer','hired'])
          ? Math.round((count(['interview','offer','hired']) / count(['shortlisted','interview','offer','hired'])) * 100) : 0,
  ...
  if (process.env.NODE_ENV !== 'production') {
    createViteServer({ server: { middlewareMode: true }, appType: 'spa' }).then((vite) => {
      ...
      app.get('*', (_req, res) => res.sendFile(path.resolve(process.cwd(), 'index.html')));
