...
// ── Helpers: map snake_case DB rows → camelCase app types ─────────────────────
...
// ── Gemini AI evaluation (unchanged from original) ────────────────────────────
...
async function dispatchEmailNotification(toEmail: string, subject: string, bodyText: string) {
  ...
  if (cfg.user && cfg.pass) {
    try {
      const transporter = nodemailer.createTransport({ host: cfg.host, port: cfg.port, secure: cfg.port === 465, auth: { user: cfg.user, pass: cfg.pass }, tls: { rejectUnauthorized: false } });
      const info = await transporter.sendMail({ from: `"Recruit-AI" <${cfg.user}>`, to: toEmail, subject, text: bodyText });
...
async function autoScheduleCandidateInterview(candidateId: string, jobId: string) {
  ...
  // Move pipeline to interview stage
  ...
  app.post('/api/screenings/evaluate', async (req, res) => {
    ...
    try {
      ...
      if (pRow) {
        ...
        const body = stage === 'shortlisted' ? `Hi ${candidate.name},\n\nYou scored ${screenRow.score}% and have been shortlisted for ${job.title}. We'll be in touch shortly.` : `Hi ${candidate.name},\n\nThank you for applying. We've decided to move forward with other candidates at this time.`;
